...
@Injectable() export class KafkaProducerService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() { const k=new Kafka({clientId:'notification-service',brokers:(process.env.KAFKA_BROKERS||'localhost:9092').split(','),retry:{retries:3}}); this.producer=k.producer(); try{await this.producer.connect();this.connected=true;}catch(e){this.logger.warn(`Kafka: ${e.message}`);} }
