-- Seed: QAST Process Definitions + Case Sequences
--
-- All 11 existing ITIL/telecom process definitions, verbatim from
-- seeds/001_core_data.sql — kept as-is per scope ("do not touch process
-- definitions beyond what's needed"). Same tenant id as before (QAST reuses
-- the canonical tenant row, see 001_tenant.sql), so no re-pointing needed.
-- Idempotent via ON CONFLICT on each table's natural key.
...
ON CONFLICT (tenant_id, slug, version) DO UPDATE
  SET name = EXCLUDED.name, description = EXCLUDED.description, category = EXCLUDED.category,
      bpmn_xml = EXCLUDED.bpmn_xml, config = EXCLUDED.config, status = EXCLUDED.status, updated_at = NOW();
